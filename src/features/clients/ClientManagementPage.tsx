...
function ClientManagementPageComponent() {
  ...
  useEffect(() => {
    ...
    const fetchClientsFromApi = async () => {
      try {
        ...
        console.info(
          "Client API endpoint not active, using cached client directory fallback.",
        );
      } finally {
      ...
      setClients((prev) =>
        prev.map((c) => {
          if (String(c.id) === String(id)) {
            targetStatus =
              c.status === "suspended" || c.status === "blacklisted"
                ? "active"
                : "suspended";
  ...
  const handleEditClientClick = useCallback((client: CorporateClient) => {
    ...
    setClientFormData({
      ...
      city:
        client.address ||
        (client.operating_cities && client.operating_cities[0]) ||
        "",
    ...
    async (e: React.FormEvent) => {
      ...
      const updatedPayload: Partial<CorporateClient> = {
        ...
        contract: {
          ...
          contractStart:
            editingClient.contract?.contractStart || "Aug 01, 2026",
      ...
      setClients((prev) =>
        prev.map((c) =>
          String(c.id) === String(editingClient.id)
            ? { ...c, ...updatedPayload }
            : c,
        ),
      );
  ...
  // Show onboarding wizard only if explicitly triggered by the user
