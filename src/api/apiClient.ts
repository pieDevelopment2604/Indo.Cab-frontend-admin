...
api.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    const currentRefreshToken = store.getState().auth.refresh_token;
    ...
    try {
      // POST /auth/refresh according to API specification
      ...
      if (!newAccessToken) {
        throw new Error("Refresh endpoint did not return a valid token");
      }
